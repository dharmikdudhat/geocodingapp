...
async function calculateDistanceUsingAPI(location1, location2) {
    try {
      ...
      if (response.data &&
        ...
        response.data.resourceSets[0].resources &&
        response.data.resourceSets[0].resources.length > 0) {
        const route = response.data.resourceSets[0].resources[0].routeLegs[0];
        const routePath = route && route.hasOwnProperty('routePath') ? route.routePath : null;
  
        if (routePath && routePath.hasOwnProperty('line') && routePath.line.hasOwnProperty('coordinates')) {
          const coordinates = routePath.line.coordinates;
          console.log('Route Coordinates:', coordinates);
          return coordinates;
        } else {
          console.log('Route coordinates not found.');
          return null;
        }
      } else {
